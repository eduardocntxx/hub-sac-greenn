  ...
  return (
    <div className="mb-4 flex flex-wrap items-baseline justify-between gap-3">
      <h2 className="flex items-center gap-2 font-display text-lg font-bold tracking-tight text-ink">
        {titulo}
        {tag === "crisp" && <Badge tone="info">Crisp</Badge>}
        {tag === "novo" && <Badge tone="success">Novo</Badge>}
      </h2>
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-white print:static print:overflow-visible">
      ...
      <div className="mx-auto max-w-[920px] px-5 py-10 print:px-0 print:py-0">
        ...
        <section className="mt-11 print:mt-8">
          <SecaoHead titulo="Preencher manualmente" nota="O Hub não tem esse dado hoje — nada foi estimado ou inventado aqui." />
          <div className="grid grid-cols-1 gap-3.5 sm:grid-cols-2 print:grid-cols-2">
            <div
              className="break-inside-avoid rounded-2xl border-[1.5px] border-dashed border-sand-line-strong p-5"
              style={{
                backgroundImage:
                  "repeating-linear-gradient(135deg, #F1F2EF, #F1F2EF 10px, #E8E8E8 10px, #E8E8E8 20px)",
              }}
            >
              <p className="text-[14.5px] font-semibold text-ink">SAC — Migrações</p>
              <p className="mt-1.5 max-w-[62ch] text-[13px] leading-relaxed text-ink/60">
                Não existe registro de migração por plataforma no banco do Hub. Cole aqui os números do controle usado hoje.
              </p>
              <div className="mt-3.5 flex flex-wrap gap-2">
                {["Finalizadas: __", "Em progresso: __", "Aguardando: __", "Por plataforma: __"].map((f) => (
                  <span key={f} className="rounded-lg border border-dashed border-sand-line-strong bg-white px-2.5 py-1 font-mono text-[11px] text-ink/40">
                    {f}
                  </span>
                ))}
              </div>
            ...
            >
              ...
              <p className="mt-1.5 max-w-[62ch] text-[13px] leading-relaxed text-ink/60">
                O módulo de NPS do Hub ainda roda com dado de exemplo. Cole aqui o resultado do acompanhamento manual de detratores.
              </p>
              <div className="mt-3.5 flex flex-wrap gap-2">
                {["Contatados: __/__", "Detratores: __", "Neutros: __", "Retorno: __/__", "Temas mais abordados: __"].map((f) => (
                  <span key={f} className="rounded-lg border border-dashed border-sand-line-strong bg-white px-2.5 py-1 font-mono text-[11px] text-ink/40">
                    {f}
                  </span>
                ))}
