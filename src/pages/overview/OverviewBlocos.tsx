...
// ---------- Indicador do topo ----------
function corDelta(d?: DeltaInfo) {
  if (!d || d.bom === null) return "text-ink/40";
  return d.bom ? "text-forest-600 dark:text-forest-300" : "text-rust-500";
}
...
export function SaudeKpi({ label, valor, delta, contexto }: { label: string; valor: string; delta?: DeltaInfo; contexto?: string }) {
  return (
    <Card className="flex flex-col gap-1 p-4">
      ...
      <span className="font-display text-[28px] font-bold leading-tight tracking-tight tabular-nums text-ink">{valor}</span>
      <span className={cn("text-xs font-semibold tabular-nums", corDelta(delta))}>{delta?.texto ?? "sem comparação"}</span>
      {contexto && <span className="text-xs text-ink/50">{contexto}</span>}
    </Card>
  );
}
...
export function PrecisaAtencao({
  ...
  return (
    <Card className="border-amber-400/50 p-5">
      <SecaoHead
      ...
      <div className="grid gap-4 lg:grid-cols-[1.25fr_1fr_1fr]">
        <div className="flex flex-col gap-2.5 rounded-2xl border border-sand-line p-4">
          <h3 className="text-[13px] font-bold text-ink">Atendido e não resolvido</h3>
          <div className="flex flex-wrap items-baseline gap-2.5">
            ...
            <span className={cn("text-xs font-semibold", corDelta(deltaAbertos))}>{deltaAbertos?.texto ?? ""}</span>
          </div>
        ...
        <div className="flex flex-col gap-2.5 rounded-2xl border border-sand-line p-4">
          <h3 className="text-[13px] font-bold text-ink">Funil do CSAT · taxa de resposta</h3>
        ...
        <div className="flex flex-col gap-2.5 rounded-2xl border border-sand-line p-4">
          <h3 className="text-[13px] font-bold text-ink">Backlog aberto agora</h3>
          <div className="flex flex-wrap items-baseline gap-2.5">
            <span className="font-display text-[26px] font-bold tabular-nums text-ink">{fmtNum(backlogTotal)}</span>
            {backlogTotal > 0 && <Badge tone="warning">{fmtPct1((backlogVelho / backlogTotal) * 100)} com +7 dias</Badge>}
          </div>
